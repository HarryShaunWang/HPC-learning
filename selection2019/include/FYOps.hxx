...
FYArray<T_numtype, N_rank>::evaluateWithStackTraversalN(T_expr expr, T_update)
{
    ...
    while ( true )
	{
        ...
        if ( (useUnitStride) || (useCommonStride) )
        {
            ...
            {//经测试, 全部都是1
				//cout << "1" << endl;
				//omp_set_num_threads(get_cpu_cores());
#pragma omp parallel
				...
				{ //ubound达到了245683308
					...
					//T_update::update(*(data + 1), expr.fastRead(i + 1));
					//T_update::update(*(data + 2), expr.fastRead(i + 2));
					//T_update::update(*(data + 3), expr.fastRead(i + 3));
					//data += 4;
				}
            }
            else 
